...
from handlers.edit_post import EditPost
from handlers.blog_front import BlogFront
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        if subject and content:
            p = Post(user_id=self.user.key().id() ,parent = blog_key(), subject = subject, content = content)
            p.put()
...
class EditPost(BlogHandler):
    def get(self):
        if not self.user:
            ...
            if post and post.post_id == self.user.key().id():
...
class Signup(BlogHandler):
    ...
    def post(self):
        ...
        ##store the username  and email in the dict so that user won't need to retype
